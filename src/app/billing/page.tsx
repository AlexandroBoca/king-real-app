    ...
    <ProtectedRoute>
      ...
      >
        <div className="max-w-7xl mx-auto">
          ...
          <div className={`${theme === 'dark' ? 'bg-gray-800' : 'bg-white'} p-4 sm:p-6 rounded-lg shadow mb-6 sm:mb-8`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {paymentMethods.map((method) => (
                <div 
                  key={method.id} 
                  className={`relative p-4 border ${theme === 'dark' ? 'border-gray-700 bg-gray-800 hover:border-indigo-400' : 'border-gray-200 bg-white hover:border-indigo-300'} rounded-lg hover:shadow-md transition-all duration-300 cursor-pointer`}
                >
                  {/* Card Header */}
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between text-sm">
                      <span className={`text-sm ${theme === 'dark' ? 'text-gray-400' : 'text-gray-500'}`}>Expires</span>
                      <span className={`text-sm font-medium ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>{method.expiry}</span>
                    </div>
                    <div className="flex items-center justify-between">
                      ...
                      <button className={`text-sm font-medium transition-colors duration-200 ${theme === 'dark' ? 'text-red-400 hover:text-red-300' : 'text-red-600 hover:text-red-700'}`}>
                        Remove
                      </button>
                    </div>
                  </div>
            ...
            <div className={`${theme === 'dark' ? 'bg-gray-800' : 'bg-white'} p-6 rounded-lg shadow`}>
              <h2 className={`text-lg font-semibold ${theme === 'dark' ? 'text-white' : 'text-gray-900'} mb-4`}>Billing Information</h2>
              <div className="space-y-3">
                <div>
                  <label className={`block text-sm font-medium ${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}`}>Billing Email</label>
                  <p className={`text-sm mt-1 ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>{user?.email}</p>
                </div>
                <div>
                  <label className={`block text-sm font-medium ${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}`}>Billing Address</label>
                  <p className={`text-sm ${theme === 'dark' ? 'text-white' : 'text-gray-900'} mt-1`}>123 Main Street</p>
                  <p className={`text-sm ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>City, State 12345</p>
                  <p className={`text-sm ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>United States</p>
                </div>
                <button className={`text-sm font-medium ${theme === 'dark' ? 'text-indigo-400 hover:text-indigo-300' : 'text-indigo-600 hover:text-indigo-500'}`}>
          ...
          {/* Billing History */}
          <div>
            <h2 className={`text-base sm:text-lg font-semibold ${theme === 'dark' ? 'text-white' : 'text-gray-900'} mb-4`}>Billing History</h2>
            <div className={`${theme === 'dark' ? 'bg-gray-800' : 'bg-white'} rounded-lg shadow overflow-hidden`}>
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className={`${theme === 'dark' ? 'bg-gray-700' : 'bg-gray-50'}`}>
                    <tr>
                      <th className={`px-4 sm:px-6 py-3 text-left text-xs font-medium ${theme === 'dark' ? 'text-gray-300' : 'text-gray-500'} uppercase tracking-wider`}>Date</th>
                      <th className={`px-4 sm:px-6 py-3 text-left text-xs font-medium ${theme === 'dark' ? 'text-gray-300' : 'text-gray-500'} uppercase tracking-wider`}>Description</th>
                      <th className={`px-4 sm:px-6 py-3 text-left text-xs font-medium ${theme === 'dark' ? 'text-gray-300' : 'text-gray-500'} uppercase tracking-wider`}>Amount</th>
                      <th className={`px-4 sm:px-6 py-3 text-left text-xs font-medium ${theme === 'dark' ? 'text-gray-300' : 'text-gray-500'} uppercase tracking-wider`}>Status</th>
                      <th className={`px-4 sm:px-6 py-3 text-left text-xs font-medium ${theme === 'dark' ? 'text-gray-300' : 'text-gray-500'} uppercase tracking-wider hidden sm:table-cell`}>Actions</th>
                    </tr>
                  </thead>
                  <tbody className={`${theme === 'dark' ? 'bg-gray-800 divide-gray-700' : 'bg-white divide-gray-200'}`}>
                    {billingHistory.map((invoice) => (
                      <tr key={invoice.id} className={`${theme === 'dark' ? 'hover:bg-gray-700' : 'hover:bg-gray-50'}`}>
                        <td className={`px-4 sm:px-6 py-4 whitespace-nowrap text-sm ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>
                          {invoice.date}
                        </td>
                        <td className={`px-4 sm:px-6 py-4 whitespace-nowrap text-sm ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>
                          {invoice.description}
                        ...
                        <td className="px-4 sm:px-6 py-4 whitespace-nowrap text-sm font-medium hidden sm:table-cell">
                          <button className={`${theme === 'dark' ? 'text-indigo-400 hover:text-indigo-300' : 'text-indigo-600 hover:text-indigo-900'}`}>Download</button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
        ...
        {showAddCardModal && (
          <div className="fixed inset-0 z-[9999] overflow-y-auto">
            <div className="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
              ...
              {/* Modal panel */}
              <div className={`inline-block align-bottom ${theme === 'dark' ? 'bg-gray-800' : 'bg-white'} rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full z-[9999] relative`}>
                <form onSubmit={handleAddCard}>
                  <div className={`${theme === 'dark' ? 'bg-gray-800' : 'bg-white'} px-4 pt-5 pb-4 sm:p-6 sm:pb-4`}>
                    <div className="sm:flex sm:items-start">
                      <div className="w-full">
                        ...
                        {/* Card Type Display */}
                        {newCard.cardType && (
                          <div className={`mb-4 p-3 ${theme === 'dark' ? 'bg-blue-900 border-blue-700' : 'bg-blue-50 border-blue-200'} border rounded-md`}>
                        ...
                        <div className="space-y-4">
                          {/* Card Number */}
                          <div>
                            <label className={`block text-sm font-medium ${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}`}>Card Number</label>
                            <input
                              type="text"
                          ...
                          {/* Cardholder Name */}
                          <div>
                            <label className={`block text-sm font-medium ${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}`}>Cardholder Name</label>
                            <input
                              type="text"
                          ...
                          <div className="grid grid-cols-2 gap-4">
                            <div>
                              <label className={`block text-sm font-medium ${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}`}>Month</label>
                              <select
                            ...
                            <div>
                              <label className={`block text-sm font-medium ${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}`}>Year</label>
                              <select
                          ...
                          {/* CVV */}
                          <div>
                            <label className={`block text-sm font-medium ${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}`}>CVV</label>
                            <input
                              type="text"
                              value={newCard.cvv}
                  ...
                  <div className={`${theme === 'dark' ? 'bg-gray-700' : 'bg-gray-50'} px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse`}>
                    <button
                      type="submit"
                      className={`w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-indigo-600 text-base font-medium text-white hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:ml-3 sm:w-auto sm:text-sm ${theme === 'dark' ? 'focus:ring-offset-gray-800' : ''}`}
                    >
                      Add Card
                    </button>
                    <button
                      type="button"
                      onClick={() => setShowAddCardModal(false)}
                      className={`mt-3 w-full inline-flex justify-center rounded-md border ${theme === 'dark' ? 'border-gray-600 bg-gray-700 text-gray-300 hover:bg-gray-600' : 'border-gray-300 bg-white text-gray-700 hover:bg-gray-50'} shadow-sm px-4 py-2 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm ${theme === 'dark' ? 'focus:ring-offset-gray-800' : ''}`}
                    >
                      Cancel
                    </button>
                  </div>
