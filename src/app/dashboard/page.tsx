              ...
              >
                <div className="p-6">
                  <div className="flex items-center justify-between mb-4">
                    <div className={`p-3 rounded-lg group-hover:scale-110 transition-transform duration-300 ${theme === 'dark' ? '' : stat.bgColor}`}>
                      {stat.icon}
                    </div>
                  ...
                  <div>
                    <p className={`text-sm font-medium ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'} mb-1`}>{stat.name}</p>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8">
            ...
            <div className={`lg:col-span-2 ${theme === 'dark' ? 'bg-gray-800' : 'bg-white'} rounded-xl shadow-sm p-6`}>
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6">
                <div>
                  <h3 className={`text-lg font-semibold ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>Revenue Overview</h3>
                  <p className={`text-sm ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'} mt-1`}>Track your revenue over time</p>
                </div>
...
　　 　 　 　 {/* Simple Chart Visualization */}
                ...
                <div className="flex items-end justify-between h-48 px-2">
                  {chartData.map((data, index) => (
                    <div key={data.name} className="flex flex-col items-center flex-1">
                      ...
                      <span className={`text-xs ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'} mt-2`}>{data.name}</span>
                    </div>
                  ))}
                </div>
            ...
            <div className={`${theme === 'dark' ? 'bg-gray-800' : 'bg-white'} rounded-xl shadow-sm p-6`}>
              <h3 className={`text-lg font-semibold ${theme === 'dark' ? 'text-white' : 'text-gray-900'} mb-6`}>Quick Actions</h3>
              <div className="space-y-4">
                {quickActions.map((action, index) => (
                  <Link
                    key={action.name}
                    href={action.href}
                    className={`group block p-4 rounded-lg border ${theme === 'dark' ? 'border-gray-700 hover:border-indigo-500' : 'border-gray-200 hover:border-indigo-300'} hover:shadow-md transition-all duration-200`}
                    style={{ animationDelay: `${index * 100}ms` }}
                  >
                    <div className="flex items-center">
                      ...
                      <div className="ml-4 flex-1">
                        <p className={`text-sm font-medium ${theme === 'dark' ? 'text-white group-hover:text-indigo-400' : 'text-gray-900 group-hover:text-indigo-600'} transition-colors duration-200`}>
                          {action.name}
                        </p>
                        <p className={`text-xs ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'} mt-1`}>{action.description}</p>
                      </div>
          ...
          <div className={`${theme === 'dark' ? 'bg-gray-800' : 'bg-white'} rounded-xl shadow-sm p-6`}>
            <div className="flex items-center justify-between mb-6">
              <div>
                ...
                <p className={`text-sm ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'} mt-1`}>Latest updates from your team</p>
              </div>
            ...
            <div className="space-y-4">
              {recentActivity.map((activity, index) => (
                <div
                  key={activity.id}
                  className={`flex items-center p-4 rounded-lg ${theme === 'dark' ? 'hover:bg-gray-700' : 'hover:bg-gray-50'} transition-colors duration-200 cursor-pointer group`}
                  style={{ animationDelay: `${index * 100}ms` }}
                >
                  ...
                  <div className="ml-4 flex-1">
                    ...
                    <p className={`text-sm ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'} mt-1`}>{activity.time}</p>
                  </div>
                  <svg className={`w-5 h-5 ${theme === 'dark' ? 'text-gray-400 group-hover:text-indigo-400' : 'text-gray-400 group-hover:text-indigo-600'} group-hover:translate-x-1 transition-all duration-200`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7m0 0l-7-7m7-7V3" />
                  </svg>
                </div>
              ))}
